    ...
    <div class="card card-body mb-4">
        <div class="search__filter mb-0">
            ...
            <div class="row align-items-center justify-content-between">
                            <div class="col-3">
                               <p class="small text-muted mt-0 mb-0">Showing {{$data->firstItem()}} - {{$data->lastItem()}} out of {{$data->total()}} Entries</p>
                            </div>
                            <div class="col-9">
                                <form action="{{ route('admin.headquaters.index')}}" method="GET">
                                ...
                                <a href="{{ url()->current() }}" class="btn btn-sm btn-light clear-filter" data-bs-toggle="tooltip" title="Clear Filter">
                                 ...
                                 {{-- <a href="#csvUploadModal" data-bs-toggle="modal" class="btn btn-danger mt-2">Bulk upload</a>--}}
                                        ...
                                        <div class="search-filter-right-el">
                                             <a href="{{ route('admin.headquaters.create') }}" class="btn btn-danger btn-sm">
                       <iconify-icon icon="prime:plus-circle"></iconify-icon> Create New
                    </a>
                                        </div>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <div class="row__action">
                                <form action="{{ route('admin.headquaters.destroy',$item->id) }}" method="POST">
                                    <a href="{{ route('admin.headquaters.edit', $item->id) }}">Edit</a>
                                    <a href="{{ route('admin.headquaters.show', $item->id) }}">View</a>
                                    <a href="{{ route('admin.headquaters.status', $item->id) }}">{{($item->status == 1) ? 'Active' : 'Inactive'}}</a>
                                    @csrf
                                    @method('DELETE')
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form method="post" action="{{ route('admin.headquaters.csv.upload') }}" enctype="multipart/form-data" id="borrowerCsvUpload">@csrf
